...
from app.schemas.errors import (
    APIErrorResponse, create_validation_error, create_authentication_error,
    create_permission_error, create_not_found_error, create_conflict_error,
    create_internal_error, format_pydantic_errors, ErrorCode
)
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    if exc.status_code == 401:
        ...
        api_error = create_permission_error(
            message=exc.detail or "Permission denied"
        )
        ...
        api_error = create_conflict_error(
            message=exc.detail or "Resource conflict"
        )
    elif exc.status_code == 422:
        api_error = create_validation_error(
            message=exc.detail or "Validation error"
        )
    elif exc.status_code == 429:
        from app.schemas.errors import create_rate_limit_error
        api_error = create_rate_limit_error(
