...
from app.models.route import RouteLeg as RouteLegModel
...
from app.services.routing import RouteLeg as RouteLegData
from app.services.routing import RoutePoint, get_routing_provider
from app.services.routing import RouteResult as RouteResultData
...
router.include_router(
    best_insertion_router, prefix="/optimization", tags=["route-optimization"]
)
...
async def compute_route(
    ...
    try:
        ...
        async def optimize_segment(seg_start, candidates, seg_end):
            ...
            if (
                hasattr(provider, "supports_matrix")
                and provider.supports_matrix()
                and candidates
            ):
                points_for_matrix = [
                    RoutePoint(
                        lat=float(seg_start.place.lat),
                        lon=float(seg_start.place.lon),
                        name=seg_start.place.name,
                    ),
                    *[
                        RoutePoint(
                    ...
                    RoutePoint(
                        lat=float(seg_end.place.lat),
                        lon=float(seg_end.place.lon),
                        name=seg_end.place.name,
                    ),
                ]
                # Compute matrix with graceful fallback
                try:
                    matrix = await provider.compute_matrix(
                        points_for_matrix, profile=request.profile, options=options
                    )
                except Exception as e:
                    logger.warning(
                        f"Matrix computation failed: {e}; falling back to greedy NN"
                    )
                ...
                try:
                    ...
                    if rows != expected or cols != expected:
                        raise ValueError(
                            f"Unexpected matrix size {rows}x{cols}, expected {expected}x{expected}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Matrix validation failed: {e}; falling back to greedy NN"
                    )
                    return sorted(candidates, key=lambda s: dist(seg_start, s))
        ...
        if request.optimize:
            if not via_fixed:
                ...
                optimized_via = (
                    await optimize_segment(start_stop, via_nonfixed, end_stop)
                    if via_nonfixed
                    else []
                )
                ...
                anchors = (
                    [start_stop] + sorted(via_fixed, key=lambda s: s.seq) + [end_stop]
                )
                optimized_order = []
            ...
            final_order = [
                start_stop,
                *[s for s in via_in_seq if s.id not in {start_stop.id, end_stop.id}],
                end_stop,
            ]
            seen = set()
            final_stops = []
        ...
        try:
            ...
            if baseline.total_min > 0:
                ...
                if (
                    detour_ratio
                    and detour_ratio > settings.ROUTE_DETOUR_RATIO_THRESHOLD
                ):
                    warnings.append(
                        {
                            "type": "detour",
                ...
                async def eval_stop(s):
                    async with sem:
                        ...
                        try:
                            test = await provider.compute_route(
                                points=test_points,
                                profile=request.profile,
                                options=options,
                            )
        ...
        raise HTTPException(
            status_code=429,
            detail="Routing temporarily unavailable due to provider rate limiting. Please try again in a minute.",
        )
    except Exception as e:
        raise HTTPException(
...
async def list_saved_routes(day_id: str, db: Session = Depends(get_db)):
    ...
    routes = (
        ...
        .order_by(
            RouteVersion.is_active.desc(),
            RouteVersion.is_primary.desc(),
            RouteVersion.version.desc(),
        )
        .all()
    )
...
@router.patch(
    "/days/{day_id}/routes/{route_version_id}/activate",
    response_model=RouteVersionSchema,
)
async def activate_route_version(
    day_id: str, route_version_id: str, db: Session = Depends(get_db)
):
    """Set a specific route version as the active (default) for the day."""
    rv = (
        db.query(RouteVersion)
        .filter(RouteVersion.id == route_version_id, RouteVersion.day_id == day_id)
        .with_for_update()
        .first()
    )
    if not rv:
...
@router.patch(
    "/days/{day_id}/routes/{route_version_id}/primary",
    response_model=RouteVersionSchema,
)
async def set_primary_route_version(
    day_id: str, route_version_id: str, db: Session = Depends(get_db)
):
    ...
    db.query(RouteVersion).filter(
        RouteVersion.day_id == day_id, RouteVersion.is_primary == True
...
@router.patch(
    "/days/{day_id}/routes/{route_version_id}", response_model=RouteVersionSchema
)
async def update_route_version(
    day_id: str,
    route_version_id: str,
    body: RouteVersionUpdate,
...
async def commit_route(
    ...
    try:
        ...
        max_version = (
            db.query(func.max(RouteVersion.version))
            .filter(RouteVersion.day_id == day_id)
            .scalar()
            or 0
        )
...
async def get_day_active_summary(day_id: str, db: Session = Depends(get_db)):
    ...
    stops = (
        ...
        .filter(
            Stop.day_id == day_id,
            Stop.kind.in_([StopKind.START, StopKind.END]),
    ...
    if rv:
        ...
        if rv.total_km is not None:
            try:
                total_km = float(rv.total_km)
            except Exception:
                pass
        if rv.total_min is not None:
            try:
                total_min = float(rv.total_min)
            except Exception:
                pass
...
async def get_bulk_active_summaries(
    body: BulkDayRouteSummaryRequest, db: Session = Depends(get_db)
):
    ...
    stops = (
        ...
        .options(joinedload(Stop.place))
        .filter(
            Stop.day_id.in_(list(valid_ids)),
            Stop.kind.in_([StopKind.START, StopKind.END]),
            Stop.deleted_at.is_(None),
        )
    ...
    rvs = (
        db.query(RouteVersion)
        .filter(
            RouteVersion.day_id.in_(list(valid_ids)), RouteVersion.is_active == True
        )
...
async def compute_day_route_breakdown_endpoint(
    ...
    try:
        ...
        if day.trip_id != request.trip_id:
            raise HTTPException(
                status_code=400, detail="Day does not belong to the specified trip"
            )
        ...
        if persist_route:
            try:
                ...
                if route_version:
                    ...
                    logger.info(
                        f"Route not persisted - no significant changes detected for day {request.day_id}"
                    )
                    breakdown.route_persisted = False
                    breakdown.persistence_reason = (
                        "No significant changes detected compared to existing route"
                    )

            except Exception as e:
                logger.warning(
                    f"Route persistence failed for day {request.day_id}: {e}"
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to compute route breakdown: {str(e)}"
        )
...
@router.post(
    ...
    **Authentication Required:** Bearer token
    ...
    responses={
        400: {
            "model": RouteOptimizationErrorResponse,
...
async def optimize_route(
    ...
    logger.info(
        f"Route optimization request from user {getattr(current_user, 'id', 'unknown')}"
    )
    ...
    logger.info(
        f"Route optimization completed: {len(result.ordered)} locations, "
