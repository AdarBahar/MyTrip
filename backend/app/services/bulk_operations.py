...
class BulkOperationService:
    ...
    async def bulk_delete(
        ...
        if len(ids) > self.limits.MAX_ITEMS_PER_REQUEST:
            errors.append(
                f"Too many items: {len(ids)}. Maximum allowed: {self.limits.MAX_ITEMS_PER_REQUEST}"
            )
            ...
            try:
                ...
                if post_delete_hook:
                    try:
                        ...
                        logger.warning(
                            f"Post-delete hook failed for {resource_id}: {e}"
                        )
                ...
                self.db.rollback()
                error_msg = (
                    "Cannot delete: resource has dependencies"
                    if not force
                    else f"Integrity error: {str(e)}"
                )
                results.append(
                    BulkOperationResultItem(
                        id=resource_id,
    ...
    async def bulk_update(
        ...
        if len(updates) > self.limits.MAX_ITEMS_PER_REQUEST:
            errors.append(
                f"Too many items: {len(updates)}. Maximum allowed: {self.limits.MAX_ITEMS_PER_REQUEST}"
            )
            return create_bulk_result([], errors)
            ...
            try:
                ...
                if allowed_fields:
                    ...
                    if len(filtered_data) != len(update_data):
                        logger.warning(
                            f"Some fields filtered out for update {resource_id}"
                        )
                ...
                if pre_update_hook:
                    try:
                        update_data = (
                            await pre_update_hook(resource, update_data) or update_data
                        )
                    except Exception as e:
                        results.append(
                            BulkOperationResultItem(
                ...
                if post_update_hook:
                    try:
                        ...
                        logger.warning(
                            f"Post-update hook failed for {resource_id}: {e}"
                        )
