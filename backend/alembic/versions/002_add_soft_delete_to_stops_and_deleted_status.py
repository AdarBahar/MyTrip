...
def upgrade():
    ...
    op.add_column('stops', sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True))
    
    # Update DayStatus enum to include DELETED
    # First, we need to alter the enum type
    op.execute("ALTER TABLE days MODIFY COLUMN status ENUM('ACTIVE', 'INACTIVE', 'DELETED') NOT NULL DEFAULT 'ACTIVE'")
...
def downgrade():
    ...
    op.execute("ALTER TABLE days MODIFY COLUMN status ENUM('ACTIVE', 'INACTIVE') NOT NULL DEFAULT 'ACTIVE'")
